...
export default function AppRealPreview() {
  ...
  const gear = useMemo(() => buildGear(classLine, level, budget, data?.items), [classLine, level, budget, data]);
  ...
  return (
    <main className="app-shell">
      <nav className="top-tabs">
        {TABS.map((tab) => <button key={tab.id} className={activeTab === tab.id ? 'top-tab active' : 'top-tab'} onClick={() => setActiveTab(tab.id)}>{tab.name}</button>)}
      </nav>

      {activeTab === 'overview' && (
        <>
          <Hero controls={controls} classLine={classLine} gender={gender} level={level} bestMonster={bestMonster} bestMap={bestMap} weapon={weapon} gear={gear} stats={stats} />
  ...
  return (
    <section className="hero-card">
      ...
      <div className="preview-card" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.05fr) minmax(140px, .95fr)', gap: 12, alignItems: 'stretch' }}>
        <div className="preview-frame real-preview-frame" style={{ minHeight: 230 }}>
        ...
        <div className="preview-meta" style={{ gridColumn: '1 / -1' }}>
          ...
          <span>{weapon?.title ?? '官方装备读取中'} · 真实预览</span>
        </div>
      </div>
    </section>
  );
...
function CompactControls(props) {
  ...
  return (
    <div style={{ marginTop: 18, border: '1px solid rgba(255,255,255,.28)', borderRadius: 18, background: 'rgba(255,255,255,.12)', padding: 12 }}>
