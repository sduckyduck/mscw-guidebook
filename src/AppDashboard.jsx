...
const TABS = ['总览', '角色', '地图', '装备', '材料'];
const TAB_IDS = ['overview', 'character', 'maps', 'gear', 'materials'];
const JOBS = [
  { id: 'warrior', name: '战士' },
...
const BUDGETS = [{ id: 'low', name: '低资金' }, { id: 'mid', name: '普通' }, { id: 'high', name: '有钱' }];
const PRIORITIES = [{ id: 'stable', name: '稳定优先' }, { id: 'exp', name: '经验优先' }, { id: 'material', name: '材料优先' }, { id: 'meso', name: '金币收益优先' }];
const JOB_LABEL = { warrior: 'Warrior', magician: 'Mage', bowman: 'Bowman', thief: 'Thief', pirate: 'Pirate' };
const WEAPON_WORDS = { warrior: ['sword', 'axe', 'blunt', 'spear', 'pole'], magician: ['wand', 'staff'], bowman: ['bow', 'crossbow'], thief: ['claw', 'dagger'], pirate: ['gun', 'knuckle'] };
...
export default function AppDashboard() {
  ...
  const maps = useMemo(() => getMapRecommendations({ classLine, level, statPlan, maps: officialData?.maps, monsters: officialData?.monsters }).slice(0, 8), [classLine, level, statPlan, officialData]);
  ...
  return (
    <main className="app-shell">
      <nav className="top-tabs">
        {TABS.map((label, index) => <button key={label} className={activeTab === TAB_IDS[index] ? 'top-tab active' : 'top-tab'} onClick={() => setActiveTab(TAB_IDS[index])}>{label}</button>)}
      </nav>
      {activeTab === 'overview' && <><Hero classLine={classLine} level={level} statPlan={statPlan} mainAttack={mainAttack} bestMonster={bestMonster} bestMap={bestMap} weapon={weapon} gear={gear} stats={stats} /><NextStep bestMonster={bestMonster} bestMap={bestMap} weapon={weapon} nextAp={classLine.primaryStat} /><Parameters editionId={editionId} setEditionId={setEditionId} classId={classId} setClassId={setClassId} branchId={branchId} setBranchId={setBranchId} gender={gender} setGender={setGender} level={level} setLevel={setLevel} budget={budget} setBudget={setBudget} priority={priority} setPriority={setPriority} availableClasses={availableClasses} classLine={classLine} /></>}
      {activeTab === 'character' && <><Parameters editionId={editionId} setEditionId={setEditionId} classId={classId} setClassId={setClassId} branchId={branchId} setBranchId={setBranchId} gender={gender} setGender={setGender} level={level} setLevel={setLevel} budget={budget} setBudget={setBudget} priority={priority} setPriority={setPriority} availableClasses={availableClasses} classLine={classLine} /><StatsCard stats={stats} nextAp={classLine.primaryStat} classLine={classLine} /></>}
...
function Hero({ classLine, level, statPlan, mainAttack, bestMonster, bestMap, weapon, gear, stats }) {
  return <section className="hero-card"><h1>Lv.{level} {classLine.name} 开荒驾驶舱</h1><p>优先打 <strong>{bestMonster?.name ?? '推荐怪物'}</strong>，当前命中 {bestMap?.canHitAll ? '稳定' : '偏紧'}，推荐地图 <strong>{bestMap?.name ?? '读取中'}</strong>。</p><div className="hero-stats"><HeroStat label="ACC" value={statPlan.derived.accuracy} /><HeroStat label={classLine.id === 'magician' ? '魔攻' : '主攻'} value={mainAttack} /><HeroStat label="HP" value={statPlan.derived.hp} /></div><div className="preview-card" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.05fr) minmax(140px, .95fr)', gap: 12, alignItems: 'stretch' }}><div className="preview-frame" style={{ minHeight: 230 }}><div className={`pixel-avatar ${classLine.id}`} style={{ transform: 'scale(1.05)' }}><div className="avatar-hat" /><div className="avatar-face" /><div className="avatar-body" /><div className="avatar-weapon" /></div></div><aside style={{ border: '1px solid var(--border)', borderRadius: 15, background: 'white', padding: 12, minWidth: 0 }}><div style={{ color: 'var(--muted)', fontWeight: 1000, fontSize: 12, marginBottom: 8 }}>角色属性</div><strong style={{ display: 'block', color: 'var(--text)', fontSize: 18, marginBottom: 10 }}>最终属性</strong><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>{stats.map(([label, value]) => <MiniStat key={label} label={label} value={value} />)}</div></aside><div className="preview-meta" style={{ gridColumn: '1 / -1' }}><strong>{classLine.name} Lv.{level}</strong><span>{weapon?.title ?? '官方装备读取中'} · 映射 {gear.length} 件</span></div></div></section>;
}

function HeroStat({ label, value }) { return <div className="hero-stat"><span>{label}</span><strong>{value}</strong></div>; }
