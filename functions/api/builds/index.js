...
export async function onRequestPost({ request, env }) {
  ...
  const title = cleanString(payload.title, 80);
  const authorName = cleanString(payload.authorName || payload.author_name || '匿名玩家', 40) || '匿名玩家';
  ...
  const description = cleanString(payload.description, 1200);
