...
const PROFESSIONS = [
  { id: 'smithing', zh: '锻造', en: 'Smithing', icon: 'SM', note: '金属板材、矿石精炼和重装材料。' },
  { id: 'weaponcrafting', zh: '武器制作', en: 'Weaponcrafting', icon: 'WP', note: '武器底材、攻击路线和高价值装备。' },
  { id: 'tailoring', zh: '裁缝', en: 'Tailoring', icon: 'TL', note: '布料、法师装备和轻甲路线。' },
  { id: 'woodcrafting', zh: '木工', en: 'Woodcrafting', icon: 'WD', note: '木料、弓弩、法杖和把手材料。' },
  { id: 'leatherworking', zh: '皮革', en: 'Leatherworking', icon: 'LT', note: '皮革、手套、飞侠/弓手相关材料。' },
  { id: 'arcforge', zh: '融合机体', en: 'Arcforge', icon: 'AF', note: '魔法催化、结晶和后期高价值节点。' },
];
...
// Local id hints are only used when AppData/items.json cannot resolve the item by name.
// The actual rendered image still goes through MsioItemIcon, the same component used by the other pages.
const ITEM_ID_HINTS = {
  screw: 4003000,
...
function translate(name) {
  return ZH[name] || ZH[String(name || '').trim()] || String(name || '').trim();
}
...
function normalizeRecipes(rawRecipes, rawMaterials) {
  ...
  const list = (rawRecipes || []).map((recipe) => ({
    ...recipe,
    output: recipe.output || recipe.result_item_name || recipe.name || `Recipe ${recipe.id}`,
    ...
    materials: (recipe.materials || []).map((material) => {
      ...
      const name = found.name || material.name || material.id;
...
function buildMatrix(recipes, bottlenecks) {
  return bottlenecks.map((row) => {
    const byProfession = Object.fromEntries(PROFESSIONS.map((item) => [item.id, 0]));
    recipes.forEach((recipe) => {
      recipe.materials.forEach((material) => {
...
function ProfessionIcon({ id }) {
  return <span className="craft-profession-emoji">{prof(id).icon}</span>;
}
...
export default function CraftingMaterialsPageFixed() {
  ...
  const bottlenecks = useMemo(() => buildBottlenecks(recipes), [recipes]);
  const bottleneckByName = useMemo(() => new Map(bottlenecks.map((row) => [norm(row.name), row])), [bottlenecks]);
  const matrixRows = useMemo(() => buildMatrix(recipes, bottlenecks), [recipes, bottlenecks]);
  const route = useMemo(() => simulate(recipes, professionId, mode, targetLevel, bottleneckByName), [recipes, professionId, mode, targetLevel, bottleneckByName]);
  ...
  const selectedProfession = prof(professionId);
  ...
  const filteredRecipes = useMemo(() => {
    ...
    return recipes.filter((recipe) => {
      ...
      if (!text) return true;
      const haystack = norm([recipe.output, translate(recipe.output), prof(recipe.profession).zh, prof(recipe.profession).en, ...recipe.materials.flatMap((material) => [material.name, material.zh])].join(' '));
      return haystack.includes(text);
    }).sort((a, b) => a.level - b.level || String(a.output).localeCompare(String(b.output))).slice(0, 48);
  }, [recipes, recipeScope, recipeType, query]);
  ...
  return <section className="craft-guide">
    <header className="craft-hero">
      <div>
        ...
        <h1>材料与锻造路线</h1>
        <p>先看推荐路线，再看缺什么、去哪刷，最后用配方库查细节。</p>
      </div>
    ...
    <section className="craft-control-card">
      <label><span>专业</span><select value={professionId} onChange={(event) => setProfessionId(event.target.value)}>{PROFESSIONS.map((row) => <option key={row.id} value={row.id}>{row.zh} · {row.en}</option>)}</select></label>
    ...
    <section id="craft-plan" className="craft-main-grid">
      <article className="craft-panel craft-summary-panel">
        <div className="craft-panel-title"><h2><ProfessionIcon id={professionId} />{selectedProfession.zh}升级策略</h2><span>{selectedProfession.en}</span></div>
        ...
        <p className="craft-note">{MODES.find((row) => row.id === mode)?.desc}。只想看最少操作时，按中间推荐路线从上往下做。</p>
      </article>
      ...
      <article className="craft-panel">
        ...
        <div className="craft-chip-list">
          {route.totalMaterials.map((material) => <button key={material.name} className={norm(activeMaterial) === norm(material.name) ? 'craft-chip active' : 'craft-chip'} onClick={() => setSelectedMaterial(material.name)}>
            <MaterialIcon name={material.name} items={data.items} size={20} />{material.zh || translate(material.name)} x{material.qty}
          </button>)}
        </div>
      </article>
    </section>
      ...
      <article className="craft-panel">
        ...
        <div className="craft-matrix-list">
          {matrixRows.slice(0, 10).map((row) => <div key={row.name} className="craft-matrix-row">
            ...
            <div className="craft-prof-pills">{PROFESSIONS.filter((item) => row.byProfession[item.id] > 0).slice(0, 4).map((item) => <em key={item.id}>{item.zh} {row.byProfession[item.id]}</em>)}</div>
          </div>)}
        </div>
      </article>
    ...
    <section id="craft-recipes" className="craft-panel">
      ...
      <div className="craft-browser-toolbar">
        <select value={recipeScope} onChange={(event) => setRecipeScope(event.target.value)}><option value="all">全部专业</option>{PROFESSIONS.map((row) => <option key={row.id} value={row.id}>{row.zh} · {row.en}</option>)}</select>
