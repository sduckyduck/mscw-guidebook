...
function extractPngId(src) {
  const match = srcPath(src).match(/\/(\d{6,9})(?:_raw)?\.png$/i);
  return match?.[1] || '';
}
...
function equipmentCandidates(rawId) {
  ...
  const folders = preferred ? [preferred, ...EQUIPMENT_FOLDERS.filter((folder) => folder !== preferred)] : EQUIPMENT_FOLDERS;
...
function setCandidate(img, type, id, candidates, index = 0) {
  ...
  img.dataset.cmsIconIndex = String(index);
